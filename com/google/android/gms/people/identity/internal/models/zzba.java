...
public class zzba implements Creator<NotesImpl> {
    static void zza(NotesImpl notesImpl, Parcel parcel, int i) {
        int zzbe = zzb.zzbe(parcel);
        Set set = notesImpl.zzbCc;
        if (set.contains(Integer.valueOf(1))) {
            zzb.zzc(parcel, 1, notesImpl.mVersionCode);
        }
        if (set.contains(Integer.valueOf(2))) {
            zzb.zza(parcel, 2, notesImpl.zzbDR, i, true);
        }
        if (set.contains(Integer.valueOf(3))) {
            zzb.zza(parcel, 3, notesImpl.mValue, true);
        }
        zzb.zzJ(parcel, zzbe);
    }
    ...
    public NotesImpl zzje(Parcel parcel) {
        String str = null;
        ...
        if (parcel.dataPosition() == zzbd) {
            return new NotesImpl(hashSet, i, metadataImpl, str);
        }
        throw new zza.zza("Overread allowed size end=" + zzbd, parcel);
    }

    public NotesImpl[] zzmZ(int i) {
        return new NotesImpl[i];
    }
}
