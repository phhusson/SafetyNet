...
public class zza {
    ...
    public static int zzbc(Parcel parcel) {
        return parcel.readInt();
    }

    public static int zzbd(Parcel parcel) {
        int zzbc = zzbc(parcel);
        int zza = zza(parcel, zzbc);
        int dataPosition = parcel.dataPosition();
        if (zzdr(zzbc) != 20293) {
            throw new zza("Expected object header. Got 0x" + Integer.toHexString(zzbc), parcel);
        }
        zzbc = dataPosition + zza;
        if (zzbc >= dataPosition && zzbc <= parcel.dataSize()) {
            return zzbc;
        }
        throw new zza("Size read is invalid start=" + dataPosition + " end=" + zzbc, parcel);
    }
