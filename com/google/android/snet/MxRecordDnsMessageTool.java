...
class MxRecordDnsMessageTool {
    ...
    private static byte[] DNS_MESSAGE_HEADER = new byte[]{(byte) -66, (byte) -17, (byte) 1, (byte) 0, (byte) 0, (byte) 1, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0};
    ...
    private static final CharsetDecoder sCharsetDecoder = Charset.forName("US-ASCII").newDecoder();
    ...
    private String parseNameLabels(ByteBuffer byteBuffer, Map<Integer, String> nameCache) {
        return parseNameLabels(byteBuffer, nameCache, Integer.MAX_VALUE);
    }
    ...
    private String parseNameLabels(ByteBuffer responseByteBuffer, Map<Integer, String> nameCache, int maxLength, Set<Integer> previousCompressionOffsets) {
        ...
        while (maxLength > 0) {
            ...
            if (indicatorByte == (byte) 0) {
                break;
            }
            byte maskedIndicatorByte = (byte) (indicatorByte & -64);
            if (maskedIndicatorByte == (byte) 0) {
            ...
            } else if (maskedIndicatorByte != INDICATOR_COMPRESSION_MASK) {
                return null;
            } else {
                ...
                if (nameEnd == null) {
                    ...
                    nameEnd = parseNameLabels(duplicateByteBuffer, nameCache, Integer.MAX_VALUE, previousCompressionOffsets);
                    if (nameEnd == null) {
                        return null;
                    }
    ...
    byte[] createSingleQueryBytes(String domain) {
        ...
        if (questionSection != null) {
            ...
            if (messageSize <= SupportMenu.USER_MASK) {
                int offset;
                bArr = new byte[messageSize];
                byte[] bArr2 = DNS_MESSAGE_HEADER;
                int length = bArr2.length;
                int i = 0;
                int offset2 = 0;
                while (i < length) {
                    offset = offset2 + 1;
                    bArr[offset2] = bArr2[i];
                    i++;
                    offset2 = offset;
                }
                offset = offset2;
    ...
    private List<Byte> createQuestionSectionBytes(String domain) {
        ...
        questionSectionBytes.add(Byte.valueOf((byte) 0));
        questionSectionBytes.add(Byte.valueOf(MX_QTYPE));
        questionSectionBytes.add(Byte.valueOf((byte) 0));
        questionSectionBytes.add(Byte.valueOf((byte) 1));
