...
class GBundle {
    ...
    private Map<String, String> mOtherValues = null;
    ...
    String getSslRedirectsTestHosts() {
        String hosts = this.mBundle.getString("snet_ssl_redirects_test_hosts");
        if (TextUtils.isEmpty(hosts)) {
            return "http://accounts.google.com,http://www.google.com,http://pubads.g.doubleclick.net";
        }
        return hosts;
    }

    String getSslHandshakeTestHosts() {
        String hosts = this.mBundle.getString("snet_ssl_handshake_test_hosts");
        if (TextUtils.isEmpty(hosts)) {
            return "accounts.google.com,www.google.com,play.google.com";
        }
        return hosts;
    }

    String getDeviceCountry() {
        return this.mBundle.getString("device_country");
    }

    List<String> getSystemPropertyNames() {
        List<String> systemPropertyNames = new ArrayList();
        String systemProperties = this.mBundle.getString("system_properties");
        if (!TextUtils.isEmpty(systemProperties)) {
            systemPropertyNames.addAll(Arrays.asList(systemProperties.split(Csv.COMMA)));
        }
        return systemPropertyNames;
    }

    List<String> getSelectedFilePathList() {
        List<String> filePathList = new ArrayList();
        String filePaths = this.mBundle.getString("selected_files");
        if (!TextUtils.isEmpty(filePaths)) {
            filePathList.addAll(Arrays.asList(filePaths.split(Csv.COMMA)));
        }
        return filePathList;
    }

    boolean getIsTargetedByGservices() {
        return this.mBundle.getBoolean("snet_is_targeted_by_gservices");
    }
}
