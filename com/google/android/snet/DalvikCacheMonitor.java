...
class DalvikCacheMonitor {
    ...
    private static final String[] PATHS_TO_MONITOR = new String[]{"/system/app", "/system/framework", "/system/priv-app"};
    private static final String SNET_DIR = "/snet";
    private static final String TAG = DalvikCacheMonitor.class.getCanonicalName();
    private final Context mContext;
    private File mDalvikCacheDir = new File(NEW_DALVIK_CACHE_DIR);
    ...
    private File dalvikCacheFile(String correspondingApkFile) {
        ...
        return new File(this.mDalvikCacheDir, String.valueOf(constructedFilename.substring(1)).concat("@classes.dex"));
    }
        ...
        if (dalvikCacheDbFile.exists() && dalvikCacheDbFile.length() != 0) {
            try {
                ...
                if (!(dalvikCacheInfoProto == null || dalvikCacheInfoProto.changedFiles == null)) {
                    for (com.google.android.snet.nano.IdleLogs.DalvikCacheInfo.FileInfo fileInfo : dalvikCacheInfoProto.changedFiles) {
    ...
    void writeDalvikCacheDb(Map<String, byte[]> map) {
        ...
        dalvikCacheInfoProto.changedFiles = new com.google.android.snet.nano.IdleLogs.DalvikCacheInfo.FileInfo[map.size()];
        int i = 0;
        for (Entry<String, byte[]> entry : map.entrySet()) {
            dalvikCacheInfoProto.changedFiles[i] = new com.google.android.snet.nano.IdleLogs.DalvikCacheInfo.FileInfo();
