            ...
            if (snetExternalDir != null) {
                ...
                if (!jpegFile.exists()) {
                    ...
                    if (jpegFile.length() != ((long) JPEG_CONTENT.length)) {
                        ...
                        info.jpegTampered = info.jpegTamperedBytes != 0;
                    }
                    try {
                        if (info.jpegTampered) {
                            info.jpegModificationTime = (new Os().getLastModificationTime(jpegFile) / SECONDS_IN_DAY) * SECONDS_IN_DAY;
                        }
                    } catch (OsException e) {
                    }
